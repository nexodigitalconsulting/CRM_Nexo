...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; font-size: 12px; line-height: 1.5; color: #333; background: white; }
    .document { max-width: 210mm; margin: 0 auto; padding: 20mm; background: white; }
    .header { display: flex; justify-content: space-between; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 2px solid #333; }
    .company-info h1 { font-size: 24px; color: #1a1a1a; }
    .company-logo img { max-height: 80px; max-width: 200px; }
    .document-info { text-align: right; }
    .document-number { font-size: 18px; font-weight: bold; color: #333; }
    .client-section { margin-bottom: 30px; }
    .section-title { font-size: 14px; font-weight: bold; color: #666; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 1px; }
    .client-details { background: #f8f8f8; padding: 15px; border-radius: 4px; }
    .services-table { width: 100%; border-collapse: collapse; margin-bottom: 30px; }
    .services-table th { background: #333; color: white; padding: 12px; text-align: left; font-weight: 600; }
    ...
    .totals-table .total-row { font-size: 16px; font-weight: bold; background: #333; color: white; }
    .notes-section { margin-top: 30px; padding: 15px; background: #f8f8f8; border-radius: 4px; }
    .footer { margin-top: 50px; padding-top: 20px; border-top: 1px solid #eee; text-align: center; color: #666; font-size: 10px; }
    .dates-section { display: flex; gap: 30px; margin-bottom: 20px; }
    .date-item { display: flex; gap: 10px; }
    ...
    @media print { body { print-color-adjust: exact; -webkit-print-color-adjust: exact; } .document { padding: 0; } }
  </style>
</head>
<body>
  <div class="document">
...
export function formatContractData(contract: Record<string, unknown>, companySettings?: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...
    client_city: client?.city || "", client_province: client?.province || "", client_postal_code: client?.postal_code || "",
    client_email: client?.email || "", client_phone: client?.phone || "",
    company_name: companySettings?.name || "", company_cif: companySettings?.cif || "",
    company_address: companySettings?.address || "", company_city: companySettings?.city || "",
    company_email: companySettings?.email || "", company_phone: companySettings?.phone || "", company_iban: companySettings?.iban || "",
    services: services?.map((s) => {
      const service = s.service as Record<string, unknown> | undefined;
      return { name: service?.name || "", quantity: s.quantity, unit_price: formatCurrency(s.unit_price as number),
        discount_percent: s.discount_percent, subtotal: formatCurrency(s.subtotal as number),
        iva_percent: s.iva_percent, iva_amount: formatCurrency(s.iva_amount as number), total: formatCurrency(s.total as number) };
    }) || [],
...
export function formatInvoiceData(invoice: Record<string, unknown>, companySettings?: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...
    notes: invoice.notes || "", status: getInvoiceStatusLabel(invoice.status as string),
    client_name: client?.name || "", client_cif: client?.cif || "", client_email: client?.email || "",
    client_iban: client?.iban || "", client_address: client?.address || "",
    company_name: companySettings?.name || "", company_cif: companySettings?.cif || "",
    company_address: companySettings?.address || "", company_email: companySettings?.email || "",
    company_phone: companySettings?.phone || "", company_iban: companySettings?.iban || "",
    services: services?.map((s) => {
      const service = s.service as Record<string, unknown> | undefined;
      return { name: service?.name || s.description || "", quantity: s.quantity, unit_price: formatCurrency(s.unit_price as number),
        discount_percent: s.discount_percent, subtotal: formatCurrency(s.subtotal as number),
        iva_percent: s.iva_percent, iva_amount: formatCurrency(s.iva_amount as number), total: formatCurrency(s.total as number) };
    }) || [],
    current_date: formatDate(new Date().toISOString()),
...
export function formatQuoteData(quote: Record<string, unknown>, companySettings?: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...
    notes: quote.notes || "", status: getQuoteStatusLabel(quote.status as string),
    client_name: client?.name || contact?.name || "", client_cif: client?.cif || "",
    client_email: client?.email || contact?.email || "", client_phone: client?.phone || contact?.phone || "",
    ...
    services: services?.map((s) => {
      const service = s.service as Record<string, unknown> | undefined;
      return { name: service?.name || "", quantity: s.quantity, unit_price: formatCurrency(s.unit_price as number),
        discount_percent: s.discount_percent, discount_amount: formatCurrency(s.discount_amount as number),
        subtotal: formatCurrency(s.subtotal as number), iva_percent: s.iva_percent,
        iva_amount: formatCurrency(s.iva_amount as number), total: formatCurrency(s.total as number) };
    }) || [],
    current_date: formatDate(new Date().toISOString()),
  };
}
