...
export const entityExportConfigs = {
  ...
  remittances: {
    columns: [
      ...
      { key: "issue_date", label: "Fecha Emisión", format: (v: string) => v ? format(new Date(v), "dd/MM/yyyy") : "" },
      { key: "collection_date", label: "Fecha Cobro", format: (v: string) => v ? format(new Date(v), "dd/MM/yyyy") : "" },
      { key: "invoice_count", label: "Nº Facturas" },
      { key: "total_amount", label: "Importe Total", format: (v: number) => v?.toFixed(2) || "0.00" },
      { key: "paid_amount", label: "Cobrado", format: (v: number) => v?.toFixed(2) || "0.00" },
      { key: "status", label: "Estado" },
      { key: "sent_to_bank_at", label: "Enviada al Banco", format: (v: string) => v ? format(new Date(v), "dd/MM/yyyy HH:mm") : "" },
      { key: "cancelled_at", label: "Anulada", format: (v: string) => v ? format(new Date(v), "dd/MM/yyyy HH:mm") : "" },
      { key: "cancelled_reason", label: "Motivo Anulación" },
    ],
    filename: "remesas",
  },
  campaigns: {
    columns: [
      { key: "campaign_number", label: "Nº Campaña", format: (v: number) => `CP-${String(v).padStart(4, "0")}` },
      { key: "name", label: "Nombre" },
      ...
      { key: "capture_date", label: "Fecha Captura", format: (v: string) => v ? format(new Date(v), "dd/MM/yyyy") : "" },
      { key: "status", label: "Estado" },
      { key: "created_at", label: "Fecha Creación", format: (v: string) => v ? format(new Date(v), "dd/MM/yyyy HH:mm") : "" },
    ],
    filename: "campañas",
  },
};
