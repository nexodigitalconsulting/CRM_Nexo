...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [created] = await db.insert(campaigns).values({
      ...
      sentAt: body.sent_at ? new Date(body.sent_at as string) : null,
      responseAt: body.response_at ? new Date(body.response_at as string) : null,
      responseChannel: body.response_channel as string | null,
      responseNotes: body.response_notes as string | null,
      lastContactAt: body.last_contact_at ? new Date(body.last_contact_at as string) : null,
      status: (body.status as string) ?? "pendiente",
      createdBy: user.id,
    }).returning();
